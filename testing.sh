...
# KOS Lite Installation Script (testing.sh)
# License: GPL-3.0
...
t_spinner() {
  # simple spinner showing progress of background jobs
  local pid=$!
  local delay=0.1
  local spinstr='|/-\\'
...
pkg install -y x11-repo termux-x11-nightly tur-repo pulseaudio proot-distro wget git sox virglrenderer-android mesa zlib box64 box86 wine wine64 wine32 & t_spinner
...
echo "Testing script complete. If all works, merge changes into Install_kos-lite.sh."
